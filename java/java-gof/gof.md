...
>    1. 在构造对象后，对象的属性值将**不可修改**（实体类不暴露set方法的情况下）
...
> 将请求封装成独立的对象，让你能灵活地管理、排队、记录或撤销这些请求
>
> 比如这个对象可能是一个函数，这个函数的具体实现作为参数传入，让接受者去执行这个函数

# 中介模式

> 定义一个中介对象，来封装一组对象直接的交互，从而将这些对象的交互都交给中介对象，避免各个对象之间的交互