...
从副本单元，可以得知，HW概念，和LEO概念

LEO:如果一个分区副本已经写入了 10 条消息，位移是 0 到 9，那么它的 LEO 就是 **10**, **每个副本都有自己的 LEO**

> LEO 何时更新

1. 一旦消息被成功写入到 Leader 副本的本地磁盘（操作系统页缓存，并非必须刷盘），Leader 副本的 LEO 会立即被更新（增加1）。
2. Follower 副本会不断地向 Leader 副本发送 `FETCH` 请求，拉取新消息
3. Follower 副本**成功将拉取到的消息写入其本地日志后**，会更新**它自己的 LEO**

## HW 

- **全称**：**High Watermark**，高水位线。
- **定义**：它是一个**位移值**，代表**已成功被所有 ISR（In-Sync Replicas，同步副本）副本复制的消息的边界**。
- **核心作用**：**定义了消费者可以读取到的消息范围。消费者只能消费到 HW 之前的消息，HW 之后的消息对消费者是不可见的。**

>  HW 是如何更新的

1. **Leader 负责管理**：分区 Leader 副本负责计算和推进 HW。
2. **跟踪 Follower 进度**：Leader 会跟踪所有 ISR 中 Follower 副本的 LEO。
3. **取最小值**：Leader 会**周期性地**计算 HW，其值为：
   `HW = min(所有 ISR 副本的 LEO)`
   - 简单来说，就是取 ISR 集合中**所有副本 LEO 的最小值**。这个最小值意味着，在这个位移之前的所有消息，都已经被 ISR 中的所有副本成功复制了。
...
1. 要启动kafka的幂等性，设置： enable.idempotence=true ，以及 ack=all 以及 retries > 1 。
   1. ack=all ：表示所有的同步副本都要响应
2. 或者：ack=0，不重试
   1. 可能会丢消息，适用于吞吐量指标重要性高于数据丢失，例如：日志收集
...
1. ack=0，不重试
   1. 生产者发送消息完，不管结果了，如果发送失败也就丢失了

2.  ack=1，leader crash
   1. 生产者发送消息完，只等待Leader写入成功就返回了，Leader分区丢失了，此时Follower没来及同
      步，消息丢失。
   2. ack=1：表示只要leader写入成功即可
   3. unclean.leader.election.enable 配置true
      1. 允许选举ISR以外的副本作为leader,会导致数据丢失，默认为false
...
![image-20251116232523990](image/kafka/image-20251116232523990.png)
...
1. 日志删除：按照一定的删除策略，将不满足条件的数据进行数据删除
2. 日志压缩：针对每个消息的 Key 进行整合，对于有相同 Key 的不同 Value 值，只保留最后一个版
   本。
3. Kafka 提供 log.cleanup.policy 参数进行相应配置，默认值： delete ，还可以选择compact 。
...
1. 日志删除任务会根据 log.retention.hours/log.retention.minutes/log.retention.ms 设定日志保留的时间节点。如果超过该设定值，就需要进行删除。
2. 默认是 7 天
3.  log.retention.ms 优先级最高

> 删除逻辑：

1. 每个分段的 `.timeindex` 存储了分段内部分消息的 “时间戳 → 偏移量” 映射
2. 对于每个分段，取其 **最大消息时间戳**（分段内最后一条消息的时间戳，存储在分段元数据中）
   1. 若该分段的最大时间戳 ≤ 过期时间阈值 → 整个分段 “完全过期”，加入候选删除列表；
3. 从日志对象中所维护日志分段的跳跃表中移除待删除的日志分段，保证没有线程对这些日志分段进行读取操作。
4. 这些日志分段所有文件添加 上 .delete 后缀。
5. 交由一个以 "delete-file" 命名的延迟任务来删除这些 .delete 为后缀的文件。延迟执行时间可以通过 file.delete.delay.ms 进行设置

### 基于日志大小

1. 日志删除任务会检查当前日志的大小是否超过设定值。设定项为 log.retention.bytes 
2. 单个日志分段的大小由 log.segment.bytes 进行设定

> 删除过程

1. Broker 遍历目标分区的所有 **非活跃分段**，累加其 `.log`/`.index`/`.timeindex` 文件的总大小（注意：仅统计非活跃分段，活跃分段不参与计算）
2. 对比 “非活跃分段总大小” 与 `log.retention.bytes`，若 **非活跃分段总大小 > log.retention.bytes**：触发删除，需删除最旧的非活跃分段，直到总大小 ≤ 阈值
...
1. 页缓存是操作系统实现的一种主要的磁盘缓存，以此用来减少对磁盘 I/O 的操作。具体来说，就是把磁盘中的数据缓存到内存中，把对磁盘的访问变为对内存的访问。
2. 如：linux swap
3. 简称mmap，简单描述其作用就是：将磁盘文件映射到内存, 用户通过修改内存就能修改磁盘文件
...
1. 生产者：如果设置ACK=-1，此时，需要等待所有的副本都拉取到数据，才返回状态给客户端
   1. 生产时，创建延迟队列，如果超时，返回异常给客户端
   2. 如果都拉取正常，返回OK给客户端
2. 消费者：设置批次大小为10，如果消息个数，没有达到相应个数，同时，消费时间在配置范围内，先不让消费者拉取到消息
