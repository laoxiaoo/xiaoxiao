...
1. 设置Xms和Xmx一样大，防止JVM重新调整堆空间大小影响性能。

```shell
-server -Xms8g -Xmx8g -Xmn4g
```

2. 设置DirectByteBuffer内存大小。当DirectByteBuffer占用达到这个值，就会触发Full GC。
   1. 这个是用于堆外内存的，与零拷贝有关，可以设置的大一点

```shell
-XX:MaxDirectMemorySize=15g
```

3. 如果不太关心RocketMQ的启动时间，可以设置pre-touch，这样在JVM启动的时候就会分配完整的页空间。
   1. 虽然jvm分配了比如8g内存，但是操作系统不一定会立马分配，有如下配置，则不同
...
4. 禁用偏向锁
   1. 因为偏向锁会额外的判断线程ID，如果并发低的情况下，可以禁用
