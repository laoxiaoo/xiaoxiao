...
1. MVCC被称多版本并发控制，保证读和写之间没有锁竞争， 即：读读，读写

2. MVCC解决的就是读写时的*线程安全*问题，线程不用去争抢**读写锁**。

3. 目前MVCC只在读已提交/重复读两种隔离级别下工作
...
一行数据中，大概内容如下：后面三个隐含字段分别对应该行的隐含ID、事务号和回滚指针

![image-20250819234401840](image/3-file/image-20250819234401840.png)
