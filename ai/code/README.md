...
**LLM（大语言模型）**：就是ChatGPT、Claude、Grok这类底层“大脑”。
...
**Agent（智能体）**：让AI不只是回答问题，而是**主动做事**：调用工具（搜索、写代码、发邮件）、制定计划、一步步执行。
...
**MCP** 是一个 **AI 大模型的标准化工具箱**。大模型可以通过这些工具与 **外界互动**，**获取信息**，并 **完成具体任务**

比如说：

在日常工作和学习中，我们经常需要与 浏览器、文件、数据库 和 代码仓库 等外部工具进行交互。在 传统方式 中，我们需要 手动截图或 复制文本，再将其 粘贴到 AI 窗口 中进行对话。

而 MCP 服务则能访问浏览器、文件系统等数据源。通过这种方式，数据能够通过 MCP 服务传输到 AI 中

例如：

我需要AI查询最近的天气，规划最近的出行

那么天气AI是不知道的，则可以通过  MCP 调用天气的API，返回AI，让AI根据这些天气数据，做出思考
...
LLM负责思考和推理，RAG负责提供准确的信息，AI Agent负责具体的执行

![](./README/20260309141918.jpg)
